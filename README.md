...
<img src="img/10대 중 9대가 슈퍼카(news).png" width="600" />

<img src="img/법인구매_개인구매비율(news).png" width="600" />
...
<img src="img/연두색번호판도입(news).png" width="600" />
...
## 1-1. 정책 개요
- 고급 수입차/슈퍼카를 **법인 명의로 등록**하여 **감가상각, 비용처리 등 세금 혜택**을 받는 관행이 문제시됐다.
...
| 웹 크롤링           | 04-09 | 04-09 | ALL                 | CSV, SQL 데이터        | Streamlit 작업     |
| FAQ 크롤링         | 04-09 | 04-09 | 변성일,장시인,장진슬 | CSV, SQL 데이터        | Streamlit 작업     |
| ERD 작성           | 04-10 | 04-10 | 변성일,장시인       | ERD 다이어그램         | 없음              |
| 데이터-DB 연동      | 04-10 | 04-10 | 장시인                | DB 데이터              | 크롤링<br>데이터 수집 |
| Streamlit 화면 설계 | 04-10 | 04-10 | 김승호,변성일        | 설계파일<br>WEB 화면     | 없음              |
| Streamlit-DB 연동  | 04-10 | 04-10 | 김승호,변성일        | DB 테이블              | Streamlit 화면     |
| Streamlit 화면 구현 | 04-10 | 04-10 | 김승호,변성일,장진슬 | Streamlit 화면         | 크롤링<br>데이터 수집 |
| 코드 취합           | 04-10 | 04-10 | ALL                 | Web, DB 데이터         | 크롤링<br>데이터 수집 |
| README.md 작성     | 04-10 | 04-10 | 장진슬                | GitHub README.md      | GitHub            |
...
### 📊 데이터 출처
- 차량 등록 현황 - 국토교통부 통계 누리 
- 차량 정보 필터 - DANAWA (다나와 자동차) 수입차 판매량
...
<img src="img/차량등록현황(st).png" width="600" />
<br/><br/>

### 3-1-2. 법인차 관련 뉴스 
<img src="img/법인관련뉴스(st).png" width="600" />
<img src="img/뉴스제목및요약(st).png" width="600" />
...
> <img src="img/판매량 월별 연도 비교 막대그래프.png" width="600" />
> <img src="img/판매량 총합 비교.png" width="600" />
<br/><br/>
...
<img src="img/다나와_수입차판매량예시.png" width="600" />
...
- 법인 명의 구매 줄고 **개인 명의 고급차 구매 비중 증가** : 법인 대신 고소득 개인 고객 대상 마케팅 전환하고 있다.
...
<img src="img/다운계약서(st).png" width="600" />
<img src="img/다운계약성행(news).png" width="600" />

⚠️ **개인 리스 전환**: 법인 대신 개인 명의 리스로 전환하는 사례 증가

<img src="img/리스꼼수(news).png" width="600" />

⚠️ **특수 계층 예외**: 대통령·국회의원·지방자치단체장 등의 차량은 예외 적용

<img src="img/대통령_국회의원제외.png" width="600" />
...
<img src="img/트위터반응수집(st).png" width="600" />

<img src="img/유투브기반워드클라우드(st).png" width="600" />
...
<img src="img/연두색번호판제도(news).png" width="600" />
...
제도 시행 이후 해당 혜택을 받지 못하게 되어 **세수 회복 가능성도 제기**됐다.
...
📌 국토부는 연두색 번호판 대상 차량 정보를 국세청과 주기적으로 공유하고, 세금 탈루 대응체계를 운영 중이다.
...
<img src="img/연두색번호판자주묻는질문(st).png" width="600" />
...
| 이름 | 한 줄 회고 |
|:------:|-------------------|
| 승호 | 부족한 실력이지만 팀원 분들이 잘 해주셔서 무사히 끝낸 것 같습니다. 감사합니다.🫡 |
| 지민 | 크롤링 지옥에 빠진 다음에 감자가 되어버렸습니다.🥔 <br> 조원들 덕분에 멋진 프로젝트 같이 진행했고 남은 날들도 다같이 화이팅합시다 💪  💪  💪|
| 성일 | 전부 처음 써보는 mysql, streamlit 이었습니다. 팀원분들 모두 열심히 협력해 잘 마무리 될 수 있었던거 같습니다 감사합니다 |
| 시인 | 어찌어찌 데이터관련 작업을 맡아 진행했지만, 많이 부족함에도 이해해주시고 도와주신 팀원분들 진심으로 감사했습니다.<br> 덕분에 많은걸 배웠고 함께 하나의 목표를 향해 달려가는 모습을 보며 새로운 재미를 느낄 수 있었고<br> 제 삶에 귀한 추억거리가 추가되는 경험이였습니다 🙂|
| 진슬 | 아무것도 모르는 저를 영차영차 끌고가준 우리팀원들에게 cheers🍻! 덕분에 코딩이 뭔지 이제 좀 배운거같아요.<br> 진짜 멋진 우리팀 수고많으셨어요💚 미래에 꼭 슈퍼카 오너하자💚|
